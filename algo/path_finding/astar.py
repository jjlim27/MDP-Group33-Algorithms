...
from default.utils import calc_vector, euclidean
from path_finding.validate_path import has_collision
...
class AStar:
    ...
    def __init__(
        ...
        self.moves = (
            ( 1,  0, DIST_FW,    Movement.FORWARD,       fwd),
            ( 1, -1, DIST_FL[2], Movement.FORWARD_LEFT,  fwd_left),
            ( 1,  1, DIST_FR[2], Movement.FORWARD_RIGHT, fwd_right),
            (-1,  0, DIST_BW,    Movement.BACKWARD,       bwd),
            (-1, -1, DIST_BL[2], Movement.BACKWARD_LEFT,  bwd_left),
            (-1,  1, DIST_BR[2], Movement.BACKWARD_RIGHT, bwd_right),
        )
        self.map = mp
        self.end = None
        self.x_bounds = None
