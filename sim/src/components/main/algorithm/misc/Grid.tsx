import {
  ...
  import { Obstacle, ObstacleDirection } from "../../../structures/Obstacle";
  import { Position, RobotDirection } from "../../../structures/Robot";
  import { convertThetaToDirection } from "./Calculation";
  ...
  export const createHTMLGrid = (
    ...
    selectedObstacleDirection: ObstacleDirection // Added to apply hover effect
  ...
  const createHTMLGridCellEmpty = (
    ...
    if (!canAddObstacle) {
      return (
        <td id={`cell-${x}-${y}`} className="border border-blue-900 w-8 h-8" />
      );
    }
    ...
    return (
      <td
        id={`cell-${x}-${y}`}
        className="border border-blue-900 w-8 h-8"
        //className="border border-blue-900 w-8 h-8 cursor-pointer hover:bg-black hover:border-t-4 hover:border-t-red-700"
  ...
  const createHTMLGridCellRobot = (
    ...
    return (
      <td
        id={`cell-${x}-${y}`}
        className={`border-2 border-blue-900 w-8 h-8 align-middle text-center ${
          type === "body" ? "bg-yellow-300" : "bg-red-400"
        }`}
      />
    );
  };
  ...
  const createHTMLGridCellObstacle = (
    ...
    if (!canChangeObstacleDirection) {
      return (
        <td
          id={`cell-${x}-${y}`}
          className={`border border-orange-900 w-8 h-8 align-middle text-center bg-black
 ${imageFaceBorderClassName}`}
        />
      );
    }
    ...
    return (
      <td
        id={`cell-${x}-${y}`}
        className={`border border-orange-900 w-8 h-8 align-middle text-center bg-black
 ${imageFaceBorderClassName} cursor-pointer hover:bg-black`}
